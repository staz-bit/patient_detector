...
def pet_detector(frame):
    ...
    cv2.putText(frame,"Out from the bed",(TL_outside[0]+10,TL_outside[1]-10),font,1,(255,20,255),3,cv2.LINE_AA) # OUTSIDE BOX
    cv2.rectangle(frame,TL_inside,BR_inside,(20,20,255),3)
    cv2.putText(frame,"Bedside",(TL_inside[0]+10,TL_inside[1]-10),font,1,(20,255,255),3,cv2.LINE_AA) # INSIDE BOX
    ...
    if (((int(classes[0][0] == 1) or (int(classes[0][0]) == 88))) and (pause == 0)):
    ...
    if inside_counter > 10:
        ...
        message = client.messages.create(
            body = 'The patient is not in bed',
            from_=twilio_number,
            to=my_number
            )
    ...
    if pause == 1:
        if detected_inside == True:
            cv2.putText(frame,'The patient is in bed',(int(IM_WIDTH*.1),int(IM_HEIGHT*.5)),font,3,(0,0,0),7,cv2.LINE_AA)
            cv2.putText(frame,'The patient is in bed',(int(IM_WIDTH*.1),int(IM_HEIGHT*.5)),font,3,(95,176,23),5,cv2.LINE_AA)

        if detected_outside == True:
            cv2.putText(frame,'The patient is not in bed',(int(IM_WIDTH*.1),int(IM_HEIGHT*.5)),font,3,(0,0,0),7,cv2.LINE_AA)
            cv2.putText(frame,'The patient is not in bed',(int(IM_WIDTH*.1),int(IM_HEIGHT*.5)),font,3,(95,176,23),5,cv2.LINE_AA)
